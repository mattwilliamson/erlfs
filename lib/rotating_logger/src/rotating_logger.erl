...
%%% 
%%% Call gen_event:add_handler/3, generally to the error_logger
%%% event manager .e.g.
%%%     gen_event:add_handler(error_logger, rotating_logger, Args).
%%%
%%% To log messages: call gen_event:notify/2 i.e.
%%%     gen_event:notify(event_logger, {warning, "Resource low"})
%%% @end
%%%-------------------------------------------------------------------
...
read_index_file(Dir, Name) ->
    io:format("Reading index file...~n"),
...
write_index_file(Dir, Name, Index) when is_number(Index) ->
    io:format("Writing to index file...~n"),
    IndexFile = filename:join([Dir, Name ++ ".index"]),
...
%% @end
open_log_file(State) ->
    io:format("Opening log file...~n"),
    Dir = State#state.dir,
    Name = State#state.name,
    CurrentNumber = State#state.current_number,
    FileName = io_lib:format("~p.~p", [list_to_atom(Name), CurrentNumber]),
...
write_log_file(State, {Class, Event}) ->
    ...
    io:format("Writing to log file... (~p+~pb/~p)~n", [LineBytes, CurrentBytes, MaxBytes]),
...
next_log_file(State) ->
    ...
    Name = State#state.name,
    CurrentNumber = State#state.current_number,
    io:format("Opening next log file...(~p)~n", [CurrentNumber]),
    MaxNumber = State#state.max_files,
    NextNumber = case CurrentNumber < MaxNumber of
		     true ->
			 CurrentNumber + 1;
		     false ->
			 0
